...
export default function ConvertPptToPdfPage() {
  ...
  async function handleConvert() {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:5001/docs/api/tools/convert-ppt-to-pdf", {
        method: "POST",
        body: formData,
      });
      ...
      if (!res.ok) {
        ...
        throw new Error(err || "Gagal convert PPT ke PDF.");
      }
      ...
      <main className="pt-16 md:pt-20">
        <section id="home" className="py-16 md:py-24 bg-white">
          <div className="min-h-screen py-20 px-6 max-w-4xl mx-auto">
            <h1 className="text-3xl font-bold mb-4">Convert PPT to PDF</h1>

            <p className="text-gray-600 mb-4">
              Unggah file PowerPoint (.ppt, .pptx) untuk dikonversi menjadi PDF.
            </p>

            {/* Input file → tampil hanya jika belum ada hasil */}
            {!downloadUrl && (
            ...
            {downloadUrl && (
              <div className="mt-8 text-center space-y-4">
                ...
                {/* Tombol Reset muncul setelah hasil keluar */}
                <Button
                  variant="secondary"
                  onClick={handleReset}
